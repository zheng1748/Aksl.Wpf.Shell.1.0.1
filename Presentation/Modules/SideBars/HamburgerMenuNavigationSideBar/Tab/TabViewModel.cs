...
namespace Aksl.Modules.HamburgerMenuNavigationSideBar.ViewModels
{
    public class TabViewModel : BindableBase
    {
        ...
        public TabItemViewModel GetActiveTabItemViewModel(TabInformation tabInformation)
        {
            var tabItemViewMode = ActiveTabItems.FirstOrDefault(ti => IsEqualsNameOrTitle(ti.Name, tabInformation.Name) || IsEqualsNameOrTitle(ti.Title, tabInformation.Title));

            return tabItemViewMode;
        }

        public TabItemViewModel GetStoreTabItemViewModel(TabInformation tabInformation)
        {
            var storeTabItemViewMode = StoreTabItems.FirstOrDefault(ti => IsEqualsNameOrTitle(ti.Name, tabInformation.Name) || IsEqualsNameOrTitle(ti.Title, tabInformation.Title));

            return storeTabItemViewMode;
        }

        public object GetStoreViewElementType(Type viewType)
        {
            var view = StoreTabItems.FirstOrDefault(ti => ti.ViewElementType == viewType);
        ...
        public bool IsActiveTabItem(TabInformation tabInformation)
        {
            ...
            var activeTabItem = ActiveTabItems.FirstOrDefault(ti => ti.IsSelected);
            var isExists = IsEqualsNameOrTitle(activeTabItem?.Name, tabInformation.Name) || IsEqualsNameOrTitle(activeTabItem?.Title, tabInformation.Title);
